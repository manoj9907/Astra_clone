// Note from Adi: This is our local fork of TradingView's embed script, which we got from
// https://s3.tradingview.com/external-embedding/embed-widget-advanced-chart.js
// We have made a few small changes to the script to make it not throw errors in the dev console
// when doing local development.
//
// Specifically, this script assumes that the parent DOM node of the
// <script> element loading the script is mounted only once, and does not get removed from the DOM.
// However, since we use React 'strict mode' in development, every component gets mounted and
// unmounted one extra time. When the script gets run after the first mount+unmount, it tries to
// use some properties its parent DOM node and throws an error because the parent is now null.
// There is no easy way I can think to resolve this while still using React strict mode, since the
// whole point of React strict mode is to help make sure component effects and rendering are pure
// (without side effects), however this whole script works as a side-effect and directly manipulates
// the DOM outside of React.
//
// We get around this by simply catching and silencing the error.
(() => {
  "use strict";
  var t = {
      151: (t, e) => {
          ...
          this.shouldListenToIframeResize &&
            (function (t, e, i) {
              ...
              if (!r)
                if (!c) {
                  // Adi: edited here
                  // If the script element doesn't have a parent node, it has been removed from
                  // the DOM tree (likely due to React strict mode), so we silently return without
                  // throwing an error.
                  return;
                } else {
                  return (
